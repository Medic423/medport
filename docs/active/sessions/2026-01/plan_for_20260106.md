...
5. ⚠️ **New Deployment - App Service Not Restarting:**
   - ✅ **Git deployment successful** - New deployment with improved timeout handling
   - ⚠️ **Issue:** No logs appearing - App Service may not have restarted after deployment
   - ✅ **Action Taken:** Restarted App Service to trigger new startup script
   - ✅ **Improvements in this deployment:**
     - Checks if `timeout` command is available
     - Falls back to background process with manual kill if `timeout` not available
     - Prevents infinite hangs (kills after 10 minutes)
     - Falls back to `npm install` if `npm ci` fails
   - ⏳ **Status:** Monitoring Azure logs after restart to verify startup script works correctly
