...
**Current Fix Applied:**
1. ✅ **Reverted workflow** - Include `node_modules` in deployment (back to original)
2. ✅ Set `SCM_DO_BUILD_DURING_DEPLOYMENT=false` - Prevent Azure from running npm install during deployment
3. ✅ Reverted startup command to default `npm start`
4. ✅ **Created custom startup.sh script** - Installs dependencies if `/node_modules` is empty after Azure's script runs
5. ✅ **Implemented lazy database initialization** - DatabaseManager only connects when first accessed, preventing startup crashes

**How It Works Now:**
- GitHub Actions builds the app WITH `node_modules` included
- Azure receives deployment package WITH `node_modules` (no npm install needed)
- Azure's startup script runs first and may remove `/node_modules` and try to extract tar.gz
- Custom `startup.sh` script runs after Azure's script and installs dependencies if needed
...
3. ✅ **Deployments triggered:**
   - All three deployments now underway:
     - `dev-be.yaml` - Backend dev deployment
     - `dev-fe.yaml` - Frontend dev deployment  
     - `azure-static-web-apps-witty-smoke-033c02b10.yml` - Static Web App deployment
   - GitHub Actions will deploy backend with `node_modules` included
   - Azure's script may remove `/node_modules` and try to extract tar.gz
   - Our `startup.sh` will install dependencies if needed
   - Backend should start successfully

4. ⏳ **Monitor deployments:**
   - **GitHub Actions:** https://github.com/Medic423/medport/actions
   - **Backend (dev-be.yaml):** Watch for build completion, then check Azure logs
   - **Frontend (dev-fe.yaml):** Should deploy normally
   - **Static Web App:** Should deploy normally
   - **Expected duration:** ~5-10 minutes per deployment

5. **After backend deployment completes:**
   - Check Azure log stream for startup script output:
     ```bash
     az webapp log tail --name TraccEms-Dev-Backend --resource-group TraccEms-Dev-USCentral
     ```
   - Look for:
     - "Removing node_modules.tar.gz..." (if tar.gz exists)
     - "node_modules is missing or empty. Installing dependencies..." (if needed)
