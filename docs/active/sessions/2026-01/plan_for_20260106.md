...
**Current Fix Applied:**
1. ✅ **Exclude node_modules from deployment** - Remove before ZIP creation (184MB too large for ZIP Deploy)
2. ✅ Set `SCM_DO_BUILD_DURING_DEPLOYMENT=false` - Prevent Azure from running npm install during deployment
3. ✅ **Created custom startup.sh script** - Installs dependencies if `/node_modules` is empty after Azure's script runs
4. ✅ **Implemented lazy database initialization** - DatabaseManager only connects when first accessed, preventing startup crashes
5. ✅ **Configured Azure startup command** - Set to `./startup.sh`
...
- ~~`npm install` may hang in Azure (we've seen this before)~~ ✅ **FIXED**
- ✅ Added 10-minute timeout to prevent infinite hangs
- ✅ Use `npm ci` when `package-lock.json` exists (faster, more reliable)
- ✅ Fallback to `npm install` with `--prefer-offline --no-audit` flags
- ✅ Ensure `package-lock.json` is generated in workflow before deployment
...
5. ⚠️ **Backend Startup Issue - Waiting for Next Deployment:**
   - ✅ Azure startup script executed successfully
   - ✅ Custom `startup.sh` script called: `PATH="$PATH:/home/site/wwwroot" ./startup.sh`
   - ✅ Script detected missing node_modules: "node_modules is missing or empty. Installing dependencies..."
   - ✅ Using `npm ci` (faster, more reliable): "Using npm ci (requires package-lock.json)..."
   - ⚠️ **Issue:** `npm ci` hung after ~2 minutes (no output for 10+ minutes)
   - ✅ **Fix Applied:** Updated startup script with better timeout handling
     - Checks if `timeout` command is available
     - Falls back to background process with manual kill if `timeout` not available
     - Prevents infinite hangs in future deployments
   - ⏳ **Status:** Waiting for next deployment to test improved timeout handling
