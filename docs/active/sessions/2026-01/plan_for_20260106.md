...
1. **Commit and push the workflow change:**
   ```bash
   git add .github/workflows/dev-be.yaml
   git commit -m "Fix: Remove node_modules before deployment to prevent Azure tar.gz issue"
   git push origin develop
   ```

2. **Monitor the deployment:**
   - Watch GitHub Actions for deployment progress
   - Verify the "Remove node_modules before deployment" step runs
   - Check that deployment package doesn't include node_modules

3. **After deployment completes:**
   - Check Azure log stream for npm install output
   - Verify backend starts successfully
   - Test endpoints: `curl https://dev-api.traccems.com/health`
   - Test login in browser: `https://dev-swa.traccems.com`

4. **If backend starts but database connection fails:**
   - Address Prisma connection pool timeout issue
   - Check database connectivity and firewall rules
   - Consider increasing Prisma connection pool settings
