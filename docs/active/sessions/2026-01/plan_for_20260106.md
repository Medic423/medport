...
**Fix Applied:**
1. ✅ Modified GitHub Actions workflow to remove `node_modules` before deployment
2. ✅ Set `SCM_DO_BUILD_DURING_DEPLOYMENT=true` so Azure runs `npm install` during deployment
3. ✅ Reverted startup command to default `npm start`
4. ✅ Removed unused custom startup script

**How It Works:**
- GitHub Actions builds the app but removes `node_modules` before deploying
- Azure receives deployment package WITHOUT `node_modules` or `tar.gz`
- Azure's Oryx doesn't detect node_modules, so no tar.gz is created
- Azure runs `npm install` during deployment (via SCM_DO_BUILD_DURING_DEPLOYMENT)
- Backend starts with properly installed dependencies
