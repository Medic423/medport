...
router.get('/hospital/:hospitalId', authenticateAdmin_1.authenticateAdmin, async (req, res) => {
    try {
        ...
        const healthcareLocation = await databaseManager_1.databaseManager.getPrismaClient().healthcareLocation.findUnique({
            where: { id: hospitalId }
        });
...
router.post('/', authenticateAdmin_1.authenticateAdmin, async (req, res) => {
    try {
        let { hospitalId, name, description, contactPhone, contactEmail, floor, room } = req.body;
        ...
        if (req.user && req.user.userType === 'HEALTHCARE') {
            const healthcareUser = await databaseManager_1.databaseManager.getPrismaClient().healthcareUser.findUnique({
            ...
            if (healthcareUser && !healthcareUser.manageMultipleLocations) {
                ...
                const userHospital = await databaseManager_1.databaseManager.getPrismaClient().hospital.findFirst({
        ...
        let hospital = await databaseManager_1.databaseManager.getPrismaClient().hospital.findUnique({
            where: { id: hospitalId }
        });
        ...
        const existingLocation = await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.findFirst({
            where: {
                hospitalId,
                name: name.trim(),
                isActive: true
        ...
        const pickup_locations = await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.create({
            data: {
...
router.put('/:id', authenticateAdmin_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, contactPhone, contactEmail, floor, room, isActive } = req.body;
        // Check if pickup location exists
        const existingLocation = await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.findUnique({
            where: { id }
        });
        if (!existingLocation) {
            return res.status(404).json({
        ...
        if (name && name.trim() !== existingLocation.name) {
            const duplicateLocation = await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.findFirst({
                where: {
        ...
        const updatedLocation = await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.update({
            where: { id },
            data: {
...
router.delete('/:id', authenticateAdmin_1.authenticateAdmin, async (req, res) => {
    try {
        ...
        const tripsUsingLocation = await databaseManager_1.databaseManager.getPrismaClient().trip.findFirst({
            where: {
                pickupLocationId: id
            }
        });
        ...
        await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.update({
            where: { id },
            data: { isActive: false }
        });
        res.json({
...
router.delete('/:id/hard', authenticateAdmin_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        // Check if pickup location exists
        const existingLocation = await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.findUnique({
        ...
        // Hard delete
        await databaseManager_1.databaseManager.getPrismaClient().pickup_locations.delete({
            where: { id }
        });
        res.json({
            success: true,
...
//# sourceMappingURL=pickupLocations.js.map