...
async function cleanTestData() {
  ...
  for (const email of testEmails) {
    ...
    let user = await prisma.healthcareUser.findUnique({ where: { email } });
    if (user) {
      // Delete related data first
      await prisma.transportRequest.deleteMany({ where: { healthcareCreatedById: user.id } });
      await prisma.healthcareLocation.deleteMany({ where: { healthcareUserId: user.id } });
      
      // Delete user
      await prisma.healthcareUser.delete({ where: { id: user.id } });
      continue;
    }

    user = await prisma.eMSUser.findUnique({ where: { email } });
    if (user) {
      // Delete related data first
      if (user.agencyId) {
        await prisma.unit.deleteMany({ where: { agencyId: user.agencyId } });
        await prisma.eMSAgency.deleteMany({ where: { id: user.agencyId } });
      }
      
      // Delete user
      await prisma.eMSUser.delete({ where: { id: user.id } });
      continue;
    }

    user = await prisma.centerUser.findUnique({ where: { email } });
    if (user) {
      // Delete user
...
async function createTestUsers() {
  ...
  users.ems = await prisma.eMSUser.create({
    data: {
...
async function createTestEMSData(users) {
  ...
  emsData.agency = await prisma.eMSAgency.create({
    data: {
      ...
      serviceArea: ['TEST_REGION'],
      capabilities: ['EMERGENCY_TRANSPORT', 'MEDICAL_TRANSPORT'],
      isActive: true,
      status: 'ACTIVE'
    }
  });

  // Update EMS user with agency ID
  await prisma.eMSUser.update({
    where: { id: users.ems.id },
    data: { agencyId: emsData.agency.id }
  });
...
async function verifyTestData(users, locations, emsData) {
  ...
  const userModels = {
    healthcare: prisma.healthcareUser,
    ems: prisma.eMSUser,
    admin: prisma.centerUser
  };

  for (const [role, user] of Object.entries(users)) {
    const foundUser = await userModels[role].findUnique({ where: { id: user.id } });
    if (!foundUser) {
  ...
  console.log(`   ✅ Healthcare location verified: ${foundHealthcareLocation.locationName}`);
