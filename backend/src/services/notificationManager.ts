...
class NotificationManager {
  ...
  async sendNotification(data: NotificationData): Promise<NotificationResult> {
    try {
      ...
      const results = await enhancedEmailService.sendNotification(
        data.userId,
        data.notificationType,
        data.emailData || { to: '', template: '', data: {} },
        data.smsData
      );
  ...
  async sendBulkNotification(
    ...
    successful: number;
    failed: number;
    results: { userId: string; result: NotificationResult }[];
  }> {
   ...
   */
  async sendTripAssignmentNotification(tripData: any, agencyIds: string[]): Promise<NotificationResult> {
    try {
      console.log('TCC_DEBUG: Sending trip assignment notification to agencies:', agencyIds);

      const emailData = {
        to: [] as string[], // Will be populated from agency emails
        ...
        priority: (tripData.priority === 'URGENT' ? 'high' : 'normal') as 'high' | 'normal' | 'low'
      };
   ...
   */
  async sendTripStatusUpdateNotification(tripData: any, hospitalUserId: string): Promise<NotificationResult> {
    try {
   ...
   */
  async getUserNotificationStats(userId: string, days: number = 30) {
    return await enhancedEmailService.getNotificationStats(userId, days);
  }

  /**
   * Get notification statistics for all users (admin)
   */
  async getAllNotificationStats(days: number = 30) {
    try {
      ...
      const stats = {
        ...
        byUser: {} as { [userId: string]: { sent: number; delivered: number; failed: number } },
        byType: {} as { [type: string]: { sent: number; delivered: number; failed: number } }
      };
      ...
      logs.forEach(log => {
        ...
        if (log.status === 'sent') stats.byUser[log.userId].delivered++;
        if (log.status === 'failed') stats.byUser[log.userId].failed++;
        ...
        if (log.status === 'sent') stats.byType[log.notificationType].delivered++;
        if (log.status === 'failed') stats.byType[log.notificationType].failed++;
      });
   ...
   */
  async testNotificationSystem(userId: string): Promise<{
    email: boolean;
    sms: boolean;
    preferences: any;
  }> {
    try {
      ...
      const emailTest = await enhancedEmailService.testEmailConnection();
      
      // Test SMS connection
      const smsTest = await enhancedEmailService.testSMSConnection();
