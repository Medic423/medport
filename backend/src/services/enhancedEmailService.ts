...
// Enhanced interfaces for notification management
interface SMSData {
  to: string;
  message: string;
...
class EnhancedEmailService {
  ...
  async sendNotification(
    ...
    const results: { email: DeliveryResult; sms: DeliveryResult } = {
      email: { success: false, channel: 'email' as const, error: 'Not attempted' },
      sms: { success: false, channel: 'sms' as const, error: 'Not attempted' }
    };
   ...
   */
  private async sendSMSWithLogging(
    smsData: SMSData, 
    userId: string, 
    notificationType: string
  ): Promise<DeliveryResult> {
    try {
      ...
      if (twilioSMSService.isServiceConfigured()) {
        console.log('TCC_DEBUG: Using Twilio for SMS delivery');
        result = await twilioSMSService.sendSMS(smsData.to, smsData.message);
      } else {
        console.log('TCC_DEBUG: Twilio not configured, using email-to-SMS gateway');
        result = await enhancedSMSService.sendSMS({
          ...smsData,
          userId,
          notificationType
        });
   ...
   */
  private async logNotificationDelivery(data: {
    userId: string;
    notificationType: string;
    channel: 'email' | 'sms';
    status: 'sent' | 'failed';
    ...
    try {
      ...
      logs.forEach(log => {
        if (log.channel === 'email') {
          ...
          if (log.status === 'sent') stats.byChannel.email.delivered++;
          if (log.status === 'failed') stats.byChannel.email.failed++;
        } else if (log.channel === 'sms') {
          stats.byChannel.sms.sent++;
          if (log.status === 'sent') stats.byChannel.sms.delivered++;
          if (log.status === 'failed') stats.byChannel.sms.failed++;
        }
      });
...
TCC_DEBUG: Request ID: {{tripId}} | Sent: {{timestamp}}
  ...
  async sendEmail(emailData: EmailData): Promise<boolean> {
    try {
      ...
      if (sendGridEmailService.isServiceConfigured()) {
        console.log('TCC_DEBUG: Using SendGrid for email delivery');
        const sendGridResult = await sendGridEmailService.sendEmail({
          to: emailData.to,
          subject: subject,
          text: text,
          html: html
  ...
  async testEmailConnection(): Promise<boolean> {
    try {
      ...
      if (sendGridEmailService.isServiceConfigured()) {
        ...
        const result = await sendGridEmailService.testConnection();
        if (result.success) {
  ...
  async testSMSConnection(): Promise<boolean> {
    try {
      ...
      const testResult = await enhancedSMSService.testSMSService();
   ...
   */
  async getSMSStats(userId?: string, days: number = 30) {
    return await enhancedSMSService.getSMSStats(userId, days);
  }

  /**
   * Get SMS cost statistics
   */
  async getSMSCostStats(): Promise<any> {
    return enhancedSMSService.getSMSCostStats();
  }

  /**
   * Get carrier information for a phone number
   */
  async getCarrierInfo(phoneNumber: string): Promise<any> {
    return enhancedSMSService.getCarrierInfo(phoneNumber);
  }

  /**
   * Send SMS with retry logic
   */
  async sendSMSWithRetry(smsData: SMSData, maxRetries: number = 3): Promise<any> {
    return await enhancedSMSService.sendSMSWithRetry(smsData, maxRetries);
  }

  /**
   * Send bulk SMS with rate limiting
   */
  async sendBulkSMS(messages: SMSData[], rateLimit: number = 10): Promise<any> {
    return await enhancedSMSService.sendBulkSMS(messages, rateLimit);
  }
  ...
  async getAllCarriers(): Promise<any> {
    return enhancedSMSService.getAllCarriers();
  }
}
