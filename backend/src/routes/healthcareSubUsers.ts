...
// List sub-users for the current healthcare organization
...
router.post('/', authenticateAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (req.user.userType === 'HEALTHCARE_ORGANIZATION_USER') {
      ...
      orgId = bodyOrgId;
      if (!orgId) return res.status(400).json({ success: false, error: 'organizationId is required when creating sub-user as admin' });
    }
    if (!orgId) return res.status(400).json({ success: false, error: 'No organization context found' });
    const db = databaseManager.getPrismaClient();
...
router.patch('/:id', authenticateAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const updated = await db.user.update({
      where: { id },
      data: { ...(typeof name === 'string' ? { name } : {}), ...(typeof isActive === 'boolean' ? { isActive } : {}) },
