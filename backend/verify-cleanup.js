...
async function verifyCleanup() {
  ...
  const trips = await prisma.transportRequest.findMany({
    ...
    trips.forEach(trip => {
      ...
      // Note: "Unknown Origin" and "Unknown Destination" are intentional placeholders
      // from the cleanup script, not actual data issues
      if (trip.fromLocation && trip.fromLocation.includes('Unknown') && trip.fromLocation !== 'Unknown Origin') {
        remainingIssues.unknownLocations++;
        issues.push('Unknown fromLocation');
      }
      
      if (trip.toLocation && trip.toLocation.includes('Unknown') && trip.toLocation !== 'Unknown Destination') {
        remainingIssues.unknownLocations++;
