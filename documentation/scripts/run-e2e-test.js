...
async function createTestTrip() {
  ...
  try {
    const response = await axios.post(`${CONFIG.baseURL}/api/trips`, tripData, {
    ...
    testState.testTripId = response.data.trip.id;
...
async function verifyTripCreation() {
  ...
  try {
    ...
    const trip = response.data.trips.find(t => t.id === testState.testTripId);
    if (!trip) {
...
async function acceptTrip() {
  ...
  try {
    const response = await axios.put(`${CONFIG.baseURL}/api/trips/${testState.testTripId}/accept`, {}, {
      headers: { Authorization: `Bearer ${testState.authTokens.ems}` },
      timeout: CONFIG.timeout
    });
...
async function testAssignmentVisibility() {
  ...
  try {
    ...
    const emsTrip = emsResponse.data.trips.find(t => t.id === testState.testTripId);
    if (!emsTrip) {
    ...
    const healthcareTrip = healthcareResponse.data.trips.find(t => t.id === testState.testTripId);
    if (!healthcareTrip) {
      throw new Error('Trip not found in healthcare dashboard');
    }
...
async function verifyAPIConsistency() {
  ...
  try {
    ...
    const tripFromList = tripsResponse.data.trips.find(t => t.id === testState.testTripId);
    const tripFromDetail = tripResponse.data.trip;
