...
interface HealthcareDestinationsProps {
  user: {
    id: string;
    email: string;
    name: string;
...
const HealthcareDestinations: React.FC<HealthcareDestinationsProps> = ({ user }) => {
  ...
  const loadDestinations = async () => {
    try {
      ...
      const response = await healthcareDestinationsAPI.getAll(params);
      if (response.data.success) {
        const transformedDestinations = response.data.data.map((destination: any) => ({
          ...destination,
  ...
  const geocodeAddress = async () => {
    ...
    console.log('TCC_DEBUG: Geocoding destination address:', {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await healthcareDestinationsAPI.update(editingDestination.id, editFormData);
      if (response.data.success) {
        await loadDestinations();
        setEditingDestination(null);
      } else {
        setEditError(response.data.error || 'Failed to update destination');
      }
    } catch (error: any) {
      console.error('Error updating destination:', error);
      setEditError(error.response?.data?.error || 'Failed to update destination');
    } finally {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this destination?')) {
      try {
        await healthcareDestinationsAPI.delete(id);
        await loadDestinations();
      } catch (err) {
  ...
  const filteredDestinations = destinations.filter(destination => {
    const matchesSearch = destination.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (destination.contactName && destination.contactName.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    const matchesStatus = filterStatus === 'all' || 
                         (filterStatus === 'active' && destination.isActive) ||
                         (filterStatus === 'inactive' && !destination.isActive);
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">My Healthcare Destinations</h1>
        <p className="mt-1 text-sm text-gray-500">
          Manage your list of healthcare destinations for patient transports.
        </p>
      </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        {filteredDestinations.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {searchTerm ? 'Try adjusting your search terms.' : 'Get started by adding a new destination.'}
            </p>
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        {destination.contactName && (
                          <div className="text-sm text-gray-900">{destination.contactName}</div>
                        )}
                        {destination.phone && (
                        ...
                        {!destination.contactName && !destination.phone && !destination.email && (
                          <span className="text-gray-400 text-sm">No contact info</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{destination.city}, {destination.state}</div>
                      <div className="text-sm text-gray-500 flex items-center">
          ...
          <div className="relative top-10 mx-auto border w-full max-w-4xl shadow-lg rounded-md bg-white max-h-[95vh] overflow-y-auto">
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium text-gray-900">Add Destination</h3>
                  <p className="mt-1 text-sm text-gray-500">
                    Add a new healthcare destination for trips
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handleAddSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    placeholder="Add any additional notes about this destination"
                  />
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  >
                    {addLoading ? 'Adding...' : 'Add Destination'}
                  </button>
                </div>
              </form>
            </div>
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Edit Destination</h3>
                <button
...
export default HealthcareDestinations;
